...
classification_df <- data.frame(UCRCode=as.factor(as.numeric(cluster_assignments)),
                                cluster=as.factor(as.numeric(cluster_assignments)))

minn_plot <- plot_usmap(regions="counties",include =c("053", "Hennepin"), 
                        data=classification_df, values="cluster") + 
  labs(title="Minneapolis",subtitle="Clusters by Arrest Rates.") + 
  scale_colour_hue() + 
